...
class CircularLinkedList(object):
    ...
    def add(self, d):
        if self.get_size() == 0:
            ...
            self.root = Node(d)
            self.root.set_next(self.root)  # make the root node circular
        else:
            # create a new node and add it after the root node
        ...
        while True:
            if this_node.get_data() == d:  # found
                if prev_node is not None:
                    ...
                    # if the root node is being removed, make the next node as the new root
